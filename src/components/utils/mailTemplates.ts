...
export const buildDispatchEmailHTML = (opts: {
  ...
  // ✅ CONDITIONAL META: only include fields that exist (no "—")
  const metaRows: string[] = [];

  if (dispatchId) {
    metaRows.push(`
      <p style="margin:0 0 6px;font-size:13px;line-height:1.6;color:#444;">
        <b>Dispatch ID:</b>
  ...
  if (deliveryPostcode) {
    metaRows.push(`
      <p style="margin:0 0 6px;font-size:13px;line-height:1.6;color:#444;">
        <b>Delivery postcode:</b> ${deliveryPostcode}
      </p>
    `);
  }
  ...
  if (referenceNumber) {
    metaRows.push(`
      <p style="margin:0;font-size:13px;line-height:1.6;color:#444;">
  ...
  const metaBlock =
    metaRows.length > 0
      ? `
        <div style="margin:12px 0 0;padding:12px;border:1px solid #eaecef;border-radius:8px;background:#fafafa;">
          ${metaRows.join("")}
        </div>
      `
      : "";
        ...
        <tr>
          <td align="center" style="padding:24px 12px;">
            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" 
              ...
              <!-- META / TRACKING (only if any meta exists) -->
              ${
                metaBlock
                  ? `
                    <tr>
